...
semantics of routed tool calls and upstream stderr diagnostics.
...
#### Scenario: Upstream stderr diagnostics are implemented
- **WHEN** the hub captures configured upstream stderr
- **THEN** the README states the `mcp_hub::upstream_stderr` target and upstream instance correlation
- **AND** the README documents default-enabled capture and per-upstream `stderr = false` discard behavior
...
#### Scenario: Upstream diagnostics roadmap item is implemented
- **WHEN** the hub implements the README's upstream diagnostics roadmap item
- **THEN** the README no longer presents that item as pending roadmap work
